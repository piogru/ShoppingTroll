  ...
  describe "#call" do
    ...
    context "given a product_id and tablespoon amount" do
      ...
      it "calculates correct values" do
        ...
        expect(results["ml"]).to eql((input * subject::TABLESPOON_TO_ML).round(2))
        expect(results["glasses"]).to eql((input * subject::TABLESPOON_TO_GLASS).round(2))
        expect(results["teaspoons"]).to eql((input * subject::TABLESPOON_TO_TEASPOON).round(2))
